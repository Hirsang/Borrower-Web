...
const { login, username, password, borrowerdashboard, generateIdCardNumber, generatePhoneNumber, generateRandomName, generateEmail, generateIdNPWPNumber, generateSIUPNumber, Data1, Data2, Data3, MaxSize, FilePDF, nama, name, generateRekeningNumber, generatenama, getNama, generateRandomNumber } = require("../../RunConfig");
...
describe('EKYC-Borrower-Personal', ()=> {
    it('Step Data Diri', ()=> {
        ...
        cy.wait(1000).get('#name').type(getNama());
    ...
    it('Step Data Perusahaan', ()=> {
        ...
        cy.wait(300).get('#directorName').type(getNama())
        cy.wait(300).get('#directorship').type('Bos Kacunk')
        cy.wait(300).get('#directorIdCardNumber').type(generateIdCardNumber())
        cy.wait(500).get('#email').type(generateEmail(8)+'@mail7.io')
        ...
        cy.wait(300).get('#businessTradePermitNumber').type(generateRandomNumber(15))
        cy.wait(300).get('#lastAmendmentDeedNumber').type('327')
        cy.wait(300).get('#tinCardNumber').type(generateRandomNumber(15))
    ...
    it('Step Data Upload Doc PIC', ()=> {
        ...
        cy.wait(300).xpath("//p[@id='alert-dialog-description']").should('be.visible');
        cy.wait(300).get('[data-test-qa="next"]').click()
        cy.wait(3000).xpath("//button[@aria-label='menu']").click()
        cy.wait(300).get('[data-test-qa="logout"]').click()
    })
    
    it('Step Data Bank', ()=>{
        login(username, password, borrowerdashboard);
        cy.wait(1500).get('#accountHolderName').type(getNama())
        cy.wait(500).get('#bankId').click()
        cy.wait(500).get('#bankId-option-19').click()
